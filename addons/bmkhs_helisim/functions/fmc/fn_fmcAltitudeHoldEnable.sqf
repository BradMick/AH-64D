...
if (_heli getVariable "bmkhs_altHoldActive" == false) then {
    ...
    if (_velClimb <= 1.016 && _velClimb >= -1.016) then {
        ...
        //de-activate.
        [_heli, "bmkhs_altHoldCollRef", (_heli getVariable "bmkhs_collectiveOutput")] call fza_fnc_updateNetworkGlobal;
        ...
        if (_curAltAGL < 435.254 && _gndSpeed < 20.577) then {
            [_heli, "bmkhs_altHoldDesiredAlt", _curAltAGL] call fza_fnc_updateNetworkGlobal;
            [_heli, "bmkhs_altHoldSubMode", "rad"] call fza_fnc_updateNetworkGlobal;
        } else {
            [_heli, "bmkhs_altHoldDesiredAlt", _curAltASL] call fza_fnc_updateNetworkGlobal;
            [_heli, "bmkhs_altHoldSubMode", "bar"] call fza_fnc_updateNetworkGlobal;
        };
        //Activate altitude hold
        [_heli, "bmkhs_altHoldActive", true] call fza_fnc_updateNetworkGlobal;
    };
} else {
    ...
    [_heli, "bmkhs_altHoldCollRef", 0.0] call fza_fnc_updateNetworkGlobal;
