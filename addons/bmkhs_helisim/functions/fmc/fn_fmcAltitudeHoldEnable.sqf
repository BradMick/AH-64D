...
if (_heli getVariable "bmkhs_altHoldActive" == false) then {
    ...
    if (_velClimb <= 1.016 && _velClimb >= -1.016) then {
        ...
        //de-activate.
        [_heli, "bmkhs_altHoldCollRef", (_heli getVariable "bmkhs_collectiveOutput")] call bmkhs_fnc_updateNetworkGlobal;
        ...
        if (_curAltAGL < 435.254 && _gndSpeed < 20.577) then {
            [_heli, "bmkhs_altHoldDesiredAlt", _curAltAGL] call bmkhs_fnc_updateNetworkGlobal;
            [_heli, "bmkhs_altHoldSubMode", "rad"] call bmkhs_fnc_updateNetworkGlobal;
        } else {
            [_heli, "bmkhs_altHoldDesiredAlt", _curAltASL] call bmkhs_fnc_updateNetworkGlobal;
            [_heli, "bmkhs_altHoldSubMode", "bar"] call bmkhs_fnc_updateNetworkGlobal;
        };
        //Activate altitude hold
        [_heli, "bmkhs_altHoldActive", true] call bmkhs_fnc_updateNetworkGlobal;
    };
} else {
